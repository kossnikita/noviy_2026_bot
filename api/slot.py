...
@router.get("/{name}", response_model=PrizeOut)
...
@router.put("/{name}", response_model=PrizeOut)
def update_prize(name: str, payload: PrizeUpdate, request: Request) -> PrizeOut:
    key = _validate_prize_name(name)
    title = _validate_prize_title(payload.title)
    with request.app.state.db.session() as s:
        p = s.scalar(select(Prize).where(Prize.name == key))
...
@router.delete("/{name}", response_model=DeletedOut)
